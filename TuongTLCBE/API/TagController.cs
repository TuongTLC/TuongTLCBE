...
public class TagController : Controller
{
    ...
    public async Task<ActionResult> CreateTag(TagInsertModel request)
    {
        ...
        object result = await _tagService.CreateTag(request, token);
    ...
    {
        object result = await _tagService.UpdateTag(request);
        return (result.GetType() == typeof(TagModel)) ? Ok(result) : BadRequest(result);
    }
    ...
    {
        object result = await _tagService.ChangeTagStatus(tagId, status);
    ...
    {
        object result = await _tagService.DeleteTag(tagId);
