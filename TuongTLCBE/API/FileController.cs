...
public class FileController : ControllerBase
{
    ...
    public async Task<IActionResult> UploadFile([FromForm] List<IFormFile> files)
    {
        ...
        var result = await _fileService.UploadFile(files, token);
    ...
    {
        var token = Request.Headers["Authorization"].ToString().Split(" ")[1];
        var result = await _fileService.GetFilesByUser(token);
    ...
    {
        var token = Request.Headers["Authorization"].ToString().Split(" ")[1];
        var result = await _fileService.DeleteFile(fileUrl, token);
        if (result is bool)
